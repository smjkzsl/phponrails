...
class AkHasAndBelongsToMany extends AkAssociation
{
    ...
    protected
    $_automatically_create_join_model_files = HAS_AND_BELONGS_TO_MANY_CREATE_JOIN_MODEL_CLASSES;
    ...
    public function &addAssociated($association_id, $options = array()) {
        ...
        'conditions' => false,
        'order' => false,
        'join_class_extends' => HAS_AND_BELONGS_TO_MANY_JOIN_CLASS_EXTENDS,
        ...
        */
        ...
        $options['join_class_name'] = empty($options['join_class_name']) ?
            join(
                array_map(array('AkInflector','classify'),
                array_map(array('AkInflector','singularize'),
                    $join_tables))) :
            $options['join_class_name'];
    ...
    public function delete(&$Associated) {
        ...
        if(!is_array($Associated)){
            ...
            foreach (array_keys($Associated) as $k){
                ...
                if($JoinObjectsToDelete = 
                    $this->JoinObject->findAllBy(
                ...
                {
			$JoinObjectsToDelete=$JoinObjectsToDelete->toArray();
        ...
        if(!$this->Owner->isNewRecord()){
            ...
            if (is_array($options['conditions'])) {
                $options['conditions'][0]=
                    trim($Associated->addTableAliasesToAssociatedSql(
                        $has_and_belongs_to_many_options['table_name'],
                        $options['conditions'][0]));
            } else {
                empty($options['conditions']) ? null:$options['conditions']=
                    trim($Associated->addTableAliasesToAssociatedSql(
                        $has_and_belongs_to_many_options['table_name']    ,
                        $options['conditions']));

            }
            empty($options['order']) ?null:$options['order']=
                trim($Associated->addTableAliasesToAssociatedSql(
                    $has_and_belongs_to_many_options['table_name'],
                    $options['order']));
            empty($options['group']) ?null:$options['group']=
                trim($Associated->addTableAliasesToAssociatedSql(
                    $has_and_belongs_to_many_options['table_name'],
                    $options['group']));
