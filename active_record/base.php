...
class AkActiveRecord extends AkAssociatedActiveRecord
{
    ...
    $_primaryKey,
    $_inheritanceColumn,
    $internationalize = ACTIVE_RECORD_INTERNATIONALIZE_MODELS_BY_DEFAULT,
    $_attributes = array(),
    ...
    public function init($attributes = array()) {
        LOG_EVENTS ? ($this->Logger = Ak::getLogger()) : null;
...
//判断是否是新记录
    ...
    */
    public function &newRecord($attributes) {
        ...
        if(ACTIVE_RECORD_SKIP_SETTING_ACTIVE_RECORD_DEFAULTS && empty($attributes)){
            return;
        }
        ...
        //设置属�?列表，从字段中取�?        $this->setAttributes($this->attributesFromColumnDefinition(),true);
    ...
    */
    public function reload() {
        ...
        */
        try{
            $id_to_find=$this->getId();
            $object = $this->find($id_to_find);
    ...
    protected function _setRecordTimestamps() {
        ...
        if($this->_newRecord && isset($this->expires_on)){
            if(isset($this->expires_at) && $this->hasColumn('expires_at')){
                $this->setAttribute('expires_at',Ak::getDate(strtotime($this->expires_at) + (defined('TIME_DIFFERENCE') ? TIME_DIFFERENCE*60 : 0)));
            }elseif(isset($this->expires_on) && $this->hasColumn('expires_on')){
                $this->setAttribute('expires_on',Ak::getDate(strtotime($this->expires_on) + (defined('TIME_DIFFERENCE') ? TIME_DIFFERENCE*60 : 0), 'Y-m-d'));
            }
        }
    ...
    */
    public function setAttribute($attribute, $value, $inspect_for_callback_child_method = ACTIVE_RECORD_ENABLE_CALLBACK_SETTERS, $compose_after_set = true) {
        if($attribute[0] == '_'){
            return false;
        }
        if($this->isFrozen()){
    ...
    */
    public function setAttributes($attributes, $override_attribute_protection = false, $inspect_for_callback_child_method = ACTIVE_RECORD_ENABLE_CALLBACK_GETTERS) {
    ...
    */
    ...
    public function getAttribute($attribute, $inspect_for_callback_child_method = ACTIVE_RECORD_ENABLE_CALLBACK_GETTERS) {
        if($attribute[0] == '_'){
            return false;
        }
        //判断是否为继承的子类�?���?��type字段
        ...
        //是否定义了取属�?的方�?        if($inspect_for_callback_child_method === true){
        ...
        if(isset($this->$attribute) || 
            ...
            !empty($this->_combinedAttributes) && $this->isCombinedAttribute($attribute))){
            if($this->hasAttribute($attribute)){
    ...
    */
    public function getAttributes($inspect_for_callback_child_method = ACTIVE_RECORD_ENABLE_CALLBACK_GETTERS) {
        $attributes = array();
        //从字段中取出属�?列表
    ...
    */
    /**
    2014-4-22 如果是SAE环境，则会自动配置，不会读取�?atabase.yml
    ...
    public function &establishConnection($specification_or_profile = DEFAULT_DATABASE_PROFILE, $force = false) {
        if($force || !$this->isConnected()){
            if(IN_SAE){
                ...
                $adapter = AkDbAdapter::getInstance(     array(
                        'type'  => "mysql",
    ...
    */
    ...
    public function setTableName($table_name = null, $check_for_existence = ACTIVE_RECORD_VALIDATE_TABLE_NAMES, $check_mode = false) {
        if(empty($table_name)){
        ...
        if($check_for_existence){
            ...
            if(!$this->_db->tableExists($table_name, true)){
                if(!$check_mode){
                    ...
                    ' Set ACTIVE_RECORD_VALIDATE_TABLE_NAMES constant to false in order to'.
    ...
    public function getOnlyAvailableAttributes($attributes) {
        ...
        if(!empty($attributes) && is_array($attributes)){
            ...
            for ($i=0; $i < $size; $i++){
                ...
                if(isset($available_attributes[$k]['name'])){
    ...
    public function getColumnsForAttributes($attributes) {
        ...
        if(!empty($attributes) && is_array($attributes)){
            ...
            foreach ($attributes as $k=>$v){
                ...
                if(isset($columns[$k]['name'])){ // -if(isset($columns[$k]['name'][$k])){ +
                    $ret_attributes[$k] = $v;
                }
            }
        }
    ...
    */
    ...
    public function readColumnSettings() {
        ...
        !$this->_runCurrentModelInstallerIfExists($column_objects)){
            // rails_migrations is the first active record to be installed, therefore the table will be created after the first run.
            if($this->getTableName() != 'rails_migrations'){
    ...
    */
    ...
    /**
    * Rails data types are mapped to phpAdodb data types
    *
    * Returns the Rails data type for an Adodb Column Object
    *
    ...
    */
    public function getRailsDataType(&$adodb_column_object) {
    ...
    private function _setColumnSettings($column_name, $column_object) {
        ...
        $this->_columnsSettings[$column_name]['type'] = $this->getRailsDataType($column_object);
    ...
    */
    private function _runCurrentModelInstallerIfExists(&$column_objects) {
        ...
        if(!defined('AVOID_AUTOMATIC_ACTIVE_RECORD_INSTALLERS') && !in_array($this->getModelName(), $installed_models)){
    ...
    */
    ...
    public function getAvailableAttributesQuoted($inspect_for_callback_child_method = ACTIVE_RECORD_ENABLE_CALLBACK_GETTERS) {
        return $this->getAttributesQuoted($this->getAttributes($inspect_for_callback_child_method));
    }

    public function getAvailableAttributesQuotedWithColumnsEscaped($inspect_for_callback_child_method = ACTIVE_RECORD_ENABLE_CALLBACK_GETTERS) {
    ...
    */
    ...
    protected function _getActAsClassName($behaviour) {
        ...
        return file_exists(ACTIVE_RECORD_DIR.DS.'behaviours'.DS.'acts_as_'.AkInflector::underscore($class_name).'.php') && !class_exists('ActsAs'.$class_name) ?
    ...
    public function &_getActAsInstance($class_name, $options) {
        if(!class_exists($class_name)){
            if(substr($class_name,0,2) == 'Ak'){
                include_once(ACTIVE_RECORD_DIR.DS.'behaviours'.DS.AkInflector::underscore(substr($class_name, 2)).'.php');
            }else{
                include_once(APP_PLUGINS_DIR.DS.AkInflector::underscore($class_name).DS.'lib'.DS.$class_name.'.php');
            }
        }
        if(!class_exists($class_name)){
    ...
    */
    ...
    protected function _enableCalculations() {
        ...
        'autoload_path' => ACTIVE_RECORD_DIR.DS.'calculations.php'
        ));
    }
    ...
    */
    ...
    protected function _enableTableInheritance() {
        ...
        'autoload_path' => ACTIVE_RECORD_DIR.DS.'table_inheritance.php'
        ));
    }
    ...
    protected function _enableLocalization() {
        ...
        'autoload_path' => ACTIVE_RECORD_DIR.DS.'localization.php'
        ));
    }
    ...
    protected function _enableFinders() {
        ...
        'autoload_path' => ACTIVE_RECORD_DIR.DS.'finders.php',
        'methods_match' => '/^find.*/',
    ...
    protected function _enableCounter() {
        ...
        'autoload_path' => ACTIVE_RECORD_DIR.DS.'counter.php'
        ));
    }
    ...
    protected function _enableCombinedAttributes() {
        ...
        'autoload_path' => ACTIVE_RECORD_DIR.DS.'combined_attributes.php'
        ));
    }
    ...
    protected function _setCacheForModelMethod($method, &$value) {
        return TEST_MODE || Ak::setStaticVar('AR_'.$this->getModelName().$method, $value);
    }
}
