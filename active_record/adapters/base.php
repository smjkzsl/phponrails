...
class AkDbAdapter
{
	...
	//var_dump($dsn);
        ...
        $this->connection = DEBUG ? NewADOConnection($dsn) : @NewADOConnection($dsn);

        if (!$this->connection){

            if(defined('DATABASE_CONNECTION_FAILURE_CALLBACK') && function_exists(DATABASE_CONNECTION_FAILURE_CALLBACK)){
                $fn = DATABASE_CONNECTION_FAILURE_CALLBACK;
                $fn();
            }
            trigger_error(Ak::t("Connection to the database failed. %dsn",
            array('%dsn'=> DEBUG ? preg_replace('/\/\/(\w+):(.*)@/i','//$1:******@', urldecode($dsn))."\n" : '')),
            ($die_on_error?E_USER_ERROR:E_USER_WARNING));
        } else {
            $this->connection->debug = DEBUG == 2;
            $this->connection->SetFetchMode(ADODB_FETCH_ASSOC);
            defined('DATABASE_CONNECTION_AVAILABLE') ? null : define('DATABASE_CONNECTION_AVAILABLE', true);
        }
    }
     ...
     * @return AkDbAdapter
     */
    static function &getInstance($database_specifications = DEFAULT_DATABASE_PROFILE, $auto_connect = true, $namespace = null) {
        ...
        if (!$Connection = Ak::getStaticVar($static_var_name)){
            ...
            $namespace = empty($namespace) ? DATABASE_SETTINGS_NAMESPACE : $namespace;
            ...
            $adapter_class_file = ACTIVE_RECORD_DIR.DS.'adapters'.DS.AkInflector::underscore($database_specifications['type']).'.php';
    ...
    */
    public function select($sql, $message = '', $options = array()) {
        ...
        $infostr='['.substr((microtime()-$options['start_time']), 0, 9).'] '.serialize($options['ActiveRecord']->getModelName());
        LOG_EVENTS && 
		$this->logger->info(
			$infostr.': '.serialize($sql));
    ...
    public function failTransaction() {
        if(DEBUG && !empty($this->connection->debug) && function_exists('ak_backtrace')){
    ...
    public function getAvailableTables($force_lookup = false) {
        ...
        !TEST_MODE && $available_tables = Ak::getStaticVar('available_tables');
        if(!$force_lookup && empty($available_tables)){
            if (($available_tables = AkDbSchemaCache::get('available_tables')) === false) {
                ...
                !TEST_MODE && Ak::setStaticVar('available_tables', $available_tables);
            }
        }
        $available_tables = $force_lookup ? $this->connection->MetaTables() : $available_tables;
        $force_lookup && !TEST_MODE && Ak::setStaticVar('available_tables', $available_tables);
