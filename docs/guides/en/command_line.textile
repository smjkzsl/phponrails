...
WHOA. With just three commands we whipped up an Rails server listening on port 3000. Go! Go right now to your browser and go to http://localhost:3000. I'll wait.
...
<shell>
$ ./mrails generate
Usage: ./mrails generate generator [options] [args]
...
<shell>
$ ./mrails generate controller
Usage: ./mrails generate controller ControllerName [options]

...
...

Example:
    ./mrails generate controller CreditCard open debit credit close
...
Modules Example:
    ./mrails generate controller 'admin/credit_card' suspend late_fee
...
<shell>
$ ./mrails generate controller Greetings hello
...
<shell>
$ ./mrails generate model
Usage: ./mrails generate model ModelName [field:type, field:type]

...

Examples:
    ./mrails generate model account
    ...
    ./mrails generate model post title:string body:text published:boolean
...
<shell>
$ ./mrails generate scaffold HighScore game:string score:integer
    exists  app/models/
    exists  app/controllers/
    exists  app/helpers/
...
The migration requires that we *migrate*, that is, run some PHP code (living in that +20081217071914_create_high_scores.php+) to modify the schema of our database. Which database? The sqlite3 database that Rails will create for you when we run the +./mrails db:migrate+ command. We'll talk more about Mrails in-depth in a little while.
...
<tt>runner</tt> runs PHP code in the context of Rails non-interactively. For instance:
...
Check it: Version numbers for PHP, pear, Rails, the Rails subcomponents, your application's folder, the current Rails environment name, your app's database adapter, and schema version! +about+ is useful when you need to ask for help, check if a security patch might affect you, or when you need some stats for an existing Rails installation.
...
Many people have created a large number different web servers in PHP, and many of them can be used to run Rails. Since version 2.3, Rails uses Rack to serve its webpages, which means that any webserver that implements a Rack handler can be used. This includes WEBrick, Mongrel, Thin, and Phusion Passenger (to name a few!).
...
INFO: For a good rundown on generators, see "Understanding Generators":http://wiki.rails.org/rails/pages/UnderstandingGenerators. A lot of its material is presented here.
...
* Inside *any* Gem installed with a "rails_generators" path, and a file ending in "_generator.php"
...
$ touch ~/.rails/generators/tutorial_test/templates/tutorial.tpl
$ touch ~/.rails/generators/tutorial_test/tutorial_test_generator.php
</shell>

We'll fill +tutorial_test_generator.php+ out with:

<php>
class TutorialTestGenerator extends Rails::Generator::Base
...
<shell>
$ cat ~/.rails/generators/tutorial_test/templates/tutorial.tpl
I'm a template!
...
<shell>
$ ./mrails generate tutorial_test arg1 arg2 arg3
      exists  public
...
h4. Mrails is PHP Make

Mrails is a standalone PHP utility that replaces the Unix utility 'make', and uses a 'makefile.php' and +../mrails+ files to build up a list of tasks. In Rails, Mrails is used for common administration tasks, especially sophisticated ones that build off of each other.

You can get a list of Mrails tasks available to you, which will often depend on your current directory, by typing +./mrails --tasks+. Each task has a description, and should help you find the thing you need.

<shell>
 mrails --tasks
(in /home/developer/commandsapp)
./mrails db:abort_if_pending_migrations       # Raises an error if there are pending migrations
./mrails db:charset                           # Retrieves the charset for the current environment's database
./mrails db:collation                         # Retrieves the collation for the current environment's database
./mrails db:create                            # Create the database defined in config/database.yml for the current RAILS_ENV
...
...
./mrails tmp:pids:clear                       # Clears all files in tmp/pids
./mrails tmp:sessions:clear                   # Clears all files in tmp/sessions
./mrails tmp:sockets:clear                    # Clears all files in tmp/sockets
</shell>

Let's take a look at some of these 80 or so mrails tasks.

h5. +db:+ Database

The most common tasks of the +db:+ Mrails namespace are +migrate+ and +create+, and it will pay off to try out all of the migration mrails tasks (+up+, +down+, +redo+, +reset+). +./mrails db:version+ is useful when troubleshooting, telling you the current version of the database.

h5. +doc:+ Documentation

If you want to strip out or rebuild any of the Rails documentation (including this guide!), the +doc:+ namespace has the tools. Stripping documentation is mainly useful for slimming your codebase, like if you're writing an Rails application for an embedded platform.

h5. +gems:+ PHP gems

You can specify which gems your application uses, and +./mrails gems:install+ will install them for you. Look at your environment.php to learn how with the *config.gem* directive.

NOTE: +gems:unpack+ will unpack, that is internalize your application's Gem dependencies by copying the Gem code into your vendor/gems directory. By doing this you increase your codebase size, but simplify installation on new hosts by eliminating the need to run +./mrails gems:install+, or finding and installing the gems your application uses.
...
In addition to the +gems:unpack+ task above, you can also unpack the Rails backend specific gems into vendor/rails by calling +./mrails rails:freeze:gems+, to unpack the version of Rails you are currently using, or +./mrails rails:freeze:edge+ to unpack the most recent (cutting, bleeding edge) version.

When you have frozen the Rails gems, Rails will prefer to use the code in vendor/rails instead of the system Rails gems. You can "thaw" by running +./mrails rails:unfreeze+.
...
You can list all the timezones Rails knows about with +./mrails time:zones:all+, which is useful just in day-to-day life.
...
h5. Miscellaneous Tasks
 ...
 +./mrails stats+ is great for looking at statistics on your code, displaying things like KLOCs (thousands of lines of code) and your code to test ratio.

 +./mrails secret+ will give you a psuedo-random key to use for your session secret.

 +./mrails routes+ will list all of your defined routes, which is useful for tracking down routing problems in your app, or giving you a good overview of the URLs in an app you're trying to get familiar with.
