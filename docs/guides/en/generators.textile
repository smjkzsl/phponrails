...
AK_APP_DIR/lib/rails_generators
AK_APP_DIR/vendor/plugins/*/lib/generators
AK_APP_DIR/vendor/plugins/*/lib/rails_generators
...
In Rails 3.0 and above, generators does not look only in the source root for templates, they also search for templates in other paths. And one of them is inside +AK_APP_DIR/lib/templates+. Since we want to customize +Rails::Generators::HelperGenerator+, we can do that by simple making a template copy inside +AK_APP_DIR/lib/templates/rails/helper+ with the name +helper.php+. So let's create such file with the following content:
