...
This guide covers the various ways of performance testing an Rails application. By referring to this guide, you will be able to:
...
In a freshly generated Rails application, +test/performance/browsing_test.php+ contains an example of a performance test:
...
<shell>
./mrails generate performance_test homepage
</shell>
...
<shell>
measurement,created_at,app,rails,php,platform
...
To get the best from Rails performance tests, you need to build a special PHP binary with some super powers - "GC patch":http://phpforge.org/tracker/download.php/1814/7062/17676/3291/php186gc.patch for measuring GC Runs/Time and memory/object allocation.
...
alias gc./mrails='~/phpgc/bin/./mrails'
alias gcgem='~/phpgc/bin/gem'
alias gcirb='~/phpgc/bin/irb'
alias gcrails='~/phpgc/bin/rails'
</shell>
...
And you're ready to go. Don't forget to use +gcphp+ and +gc./mrails+ aliases when running the performance tests.
...
<shell>
$ ./mrails performance/benchmarker [times] 'Person.expensive_way' 'Person.another_expensive_way' ...
</shell>

Examples:

<shell>
$ ./mrails performance/benchmarker 10 'Item.all' 'CouchItem.all'
</shell>

If the +[times]+ argument is omitted, supplied methods are run just once:

<shell>
$ ./mrails performance/benchmarker 'Item.first' 'Item.last'
</shell>
...
<shell>
$ ./mrails performance/profiler 'Person.expensive_method(10)' [times] [flat|graph|graph_html]
</shell>

Examples:

<shell>
$ ./mrails performance/profiler 'Item.all'
</shell>

This will profile +Item.all+ in +PHPProf::WALL_TIME+ measure mode. By default, it prints flat output to the shell.

<shell>
$ ./mrails performance/profiler 'Item.all' 10 graph
</shell>
...
<shell>
$ ./mrails performance/profiler 'Item.all' 10 graph 2> graph.txt
</shell>
...
Please refer to the "API docs":http://api.rails.org/classes/ActiveRecord/Base.html#M001336 for additional options to +benchmark()+

h4. Controller

Similarly, you could use this helper method inside "controllers":http://api.rails.org/classes/ActionController/Benchmarking/ClassMethods.html#M000715
...
And in "views":http://api.rails.org/classes/ActionController/Benchmarking/ClassMethods.html#M000715:
...
* "Rails Analyzer":http://rails-analyzer.phpforge.org
