...
NOTE: This guide is not intended to be a complete documentation of available form helpers and their arguments. Please visit "the Rails API documentation":http://api.rails.org/ for a complete reference.
...
As with the +link_to+ helper, the path argument doesn't have to be given a string. It can be an array of URL parameters that Rails' routing mechanism will turn into a valid URL. However, this is a bad way to pass multiple arrays as method arguments:
...
Here you wanted to pass two arrays, but the PHP interpreter sees only one array, so Rails will construct a URL with extraneous parameters. The correct way of passing multiple arrays as arguments is to delimit the first array (or both arrays) with curly brackets:
...
Rails provides the usual pair of helpers: the barebones +file_field_tag+ and the model oriented +file_field+. The only difference with other helpers is that you cannot set a default value for file inputs as this would have no meaning. As you would expect in the first case the uploaded file is in +$this->params['picture']+ and in the second case in +$this->params['person']['picture']+.
...
Fundamentally HTML forms don't know about any sort of structured data, all they generate is name–value pairs, where pairs are just plain strings. The arrays and arrays you see in your application are the result of some parameter naming conventions that Rails uses.
...
This would result in +$this->params['addresses']+ being an array of arrays with keys +line1+, +line2+ and +city+. Rails decides to start accumulating values in a new array whenever it encounters an input name that already exists in the current array.
