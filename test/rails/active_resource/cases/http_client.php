...
class HttpClient_TestCase extends ActiveResourceUnitTest
{
    ...
    public function test_should_get_response_header() {
        ...
        $this->assertEqual($this->Client->getResponseHeader('x-test-header'), 'rails');
    }
    ...
    public function test_should_set_user_agent() {
        $this->assertEqual($this->Client->get($this->url.'/get_user_agent'), 'Rails PHP Framework AkHttpClient (http://rails.org)');
        $this->assertEqual(Ak::url_get_contents($this->url.'/get_user_agent'), 'Rails PHP Framework AkHttpClient (http://rails.org)');
