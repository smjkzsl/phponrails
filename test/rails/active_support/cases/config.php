...
class Config_TestCase extends  ActiveSupportUnitTest
{
    ...
    public function test_write_cache() {
        $expectedFileName = $this->_base_config_path.DS.ENVIRONMENT.DS.'testconfig1.php';
        $config = array('test1' => 1, 'test2' => array('test3' => 3));
        $this->Config->writeCache($config, 'testconfig1', ENVIRONMENT, true);
    ...
    public function test_read_cache() {
        $config = array('test1'=>1,'test2'=>array('test3'=>3));
        $this->Config->writeCache($config, 'testconfig1', ENVIRONMENT, true);
        $cachedConfig = $this->Config->readCache('testconfig1', ENVIRONMENT, true);
        $this->assertEqual($config, $cachedConfig);
    }
