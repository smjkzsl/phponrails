...
class Reflection_TestCase extends ActiveSupportUnitTest
{
    public function setUp() {
        /**
         * TEST_DIR.DS.'fixtures'.DS.'data'.DS.'reflection_test_class.php'
         */
 ...
 */', trim($functionStructure[0]['docBlock']));
    ...
    public function test_parse_require_include() {
        $function = '
        require_once(APP_DIR.DS.\'require_once.php\');
        require(APP_DIR.DS.\'require.php\');
        include_once(APP_DIR.DS.\'include_once.php\');
        include(APP_DIR.DS.\'include.php\');';
        $this->Reflection->parse($function);
        $structure = $this->Reflection->getDefinitions();
        $this->assertEqual(array("APP_DIR.DS.'require_once.php'"),$structure['require_once']);
        $this->assertEqual(array("APP_DIR.DS.'require.php'"),$structure['require']);
        $this->assertEqual(array("APP_DIR.DS.'include_once.php'"),$structure['include_once']);
        $this->assertEqual(array("APP_DIR.DS.'include.php'"),$structure['include']);

    }
    public function test_parse_class() {
        $function = '
        require_once(APP_DIR.DS.\'test.php\');
        /**
         * comment
         */
...
class testClass extends base {}'),trim($classStructure[0]['toString']));
        $this->assertEqual(array("APP_DIR.DS.'test.php'"),$classStructure['require_once']);
