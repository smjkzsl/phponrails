...
class FileHandling_TestCase extends ActiveSupportUnitTest
{
    ...
    public function Test_directory_delete() {
        ...
        clearstatcache();
        $this->assertFalse(is_dir(BASE_DIR.DS.'ak_test_folder'), 'Did not remove empty dir ./ak_test_folder');
        $this->assertFalse(is_dir(BASE_DIR.DS.'ak_test_folder_copy'), 'Did not remove nested dir ./ak_test_folder_copy');
    }
    ...
    public function test_mime_type_detection() {
        ...
        $this->assertTrue(in_array(Ak::mime_content_type(PUBLIC_DIR.DS.'images'.DS.'rails_framework_logo.png'),array('image/png','image/x-png')));
    ...
    public function test_dir_should_not_recurse_when_set_to_false() {
        $files_and_dirs = AkFileSystem::dir(RAILS_DIR, array('dirs' => true, 'recurse' => false));
