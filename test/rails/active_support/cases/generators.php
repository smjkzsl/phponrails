...
class ControllerGenerator_TestCase extends ActiveSupportUnitTest
{
    ...
    public function runGeneratorCommand($command){
        ...
        $Generator->setFileOptions(array('base_path'=>RAILS_DIR));
    ...
    public function test_should_generate_controller_and_auxiliary_files(){
        ...
        AkFileSystem::file_put_contents(AkConfig::getDir('views').DS.'credit_card'.DS.'credit.html.tpl', 'foo', array('base_path'=>RAILS_DIR));
        clearstatcache();

        $this->assertPattern('/collisions/', $this->runGeneratorCommand('controller CreditCard open debit credit close'));

        AkFileSystem::file_delete(AkConfig::getDir('views').DS.'credit_card'.DS.'credit.html.tpl', array('base_path'=>RAILS_DIR));
