...
include_once(CONTRIB_DIR.DS.'pear'.DS.'Mail'.DS.'mimeDecode.php');
...
class AkMailEncoding extends Mail_mimeDecode
{
    ...
    protected function _decodeHeader($encoded_header) {
        ...
        // override it using the Rails charset transcoding engine to get the result
        // as UTF-8
        ...
        if(preg_match_all('/(\=\?([^\?]+)\?([BQ]{1})\?([^\?]+)\?\=?)+/i',$encoded_header,$match)){
            foreach ($match[0] as $k=>$encoded){
                ...
                $decoded_part = trim(Ak::recode($decode_function($match[4][$k]), ACTION_MAILER_DEFAULT_CHARSET, $charset, true));
        ...
        foreach ($addresses as $address){
            ...
            if($names && !empty($address_description)){
                ...
                if(!$this->_isAscii($address_description)){
                    $address_description = '=?'.ACTION_MAILER_DEFAULT_CHARSET.'?Q?'.$this->quoted_printable_encode($address_description, 0).'?=';
                }
            }
            $headers .= (!empty($headers)?','.MAIL_HEADER_EOL.' ':'').$address_description.$address;
        }

        return empty($headers) ? false : (!empty($header_name) ? $header_name.': '.$headers.MAIL_HEADER_EOL : $headers);
    }

    protected function _isValidAddress($email) {
        return preg_match(EMAIL_REGULAR_EXPRESSION, $email);
    }
}
