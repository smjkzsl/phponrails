...
class AkActionMailer extends AkBaseModel
{
    ...
    public $delivery_method = ACTION_MAILER_DELIVERY_METHOD;
    public $perform_deliveries = true;
    public $deliveries = array();
    public $default_charset = ACTION_MAILER_DEFAULT_CHARSET;
     ...
     *
     * APP_VIEWS_DIR/mailer_name/method_name.tpl
     *
     * To render a multi/part message place templates with the content-type in the mailer_name folder:
     *
     * APP_VIEWS_DIR/mailer_name/method_name.text.html.tpl
     * APP_VIEWS_DIR/mailer_name/method_name.text.plain.tpl
     *
    ...
    public function urlFor() {
        ...
        if(isset($args[0]['base_url'])){
            $base_url = rtrim(preg_replace('/^(?!http[s]?:\/\/)(.+)/','http://$1', (strstr($args[0]['base_url'],'.')?$args[0]['base_url']:Ak::getSetting('mailer', 'base_url', HOST))),'/');
            unset($args[0]['base_url']);
        }else{
            $base_url = Ak::getSetting('mailer', 'base_url', 'http://'.HOST);
        }
    ...
    protected function _deliverUsingMailDeliveryMethod($method, &$Message, $options) {
        ...
        $handler_path = ACTION_MAILER_DIR.DS.'delivery'.DS.$file_name.'.php';
