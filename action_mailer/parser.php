...
class AkMailParser
{
    ...
    public $recode_to_charset = ACTION_MAILER_DEFAULT_CHARSET;
    public $raw_message = '';
    ...
    protected function _getStylesheetPath($path) {
        if(preg_match('/^http(s)?:\/\//', $path)){
            ...
            if(empty($path_info['extension'])){ // no extension, we don't do magic stuff
                ...
                $local_path = TMP_DIR.DS.'mailer'.DS.'remote_css'.DS.md5($base_file_name['dirname']).DS.$base_file_name.'.'.$path_info['extension'];
        ...
        $path = PUBLIC_DIR.Ak::sanitize_include($path);
