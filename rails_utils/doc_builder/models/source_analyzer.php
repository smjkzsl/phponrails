...
{
    public $dir = RAILS_DIR;
    public $base_dir = RAILS_DIR;
    public $db = false;
	...
	try{
		$SourceFile = @$FileInstance->findFirstBy('path', $file_path);
    ...
    public function indexFiles()
    {
        ...
        $FileInstance->init();
        try{
		($UnIndexedPages = $FileInstance->findAllBy('has_been_analyzed', false));
            ...
            foreach ($UnIndexedPages as $k){
                $this->log('Analyzing file '.$k->path);

                $Component = $ComponentInstance->updateComponentDetails($k, $this);
                $Classes = $ClassInstance->updateClassDetails($k, $Component, $this);

                if(!empty($Classes)){
                    //AkDebug::debug($Classes);
                }

                $k->set('has_been_analyzed', true);
                $k->save();
            }
        }catch(Exception $e){
	}
