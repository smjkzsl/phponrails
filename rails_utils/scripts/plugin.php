...
EXAMPLES
  ...
  Install a plugin from a subversion URL:
    {$script_name} install http://svn.rails.org/plugins/acts_as_versioned
  ...
  List plugins in the specified repository:
    {$script_name} list --source=http://svn.rails.org/plugins/
  ...
  Add a new repository to the source list:
    {$script_name} source http://svn.rails.org/plugins/

  Remove a repository from the source list:
    {$script_name} unsource http://svn.rails.org/plugins/
    ...
    foreach ($plugins as $plugin){
        ...
        if(!is_dir($plugin)){
            if (!@$PluginManager->getRepositoryForPlugin($plugin_name, $repository)){
                is_null($repository) ? $repository = PLUGINS_MAIN_REPOSITORY : null;
...
if($command == 'test') {
    ...
    $test_file = PLUGINS_DIR.DS.$plugin_name.DS.'test'.DS.$plugin_name.'.php';
...
if($command == 'help') {
    ...
    $help_file = PLUGINS_DIR.DS.$plugin_name.DS.'README';
