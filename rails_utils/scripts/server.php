...
defined('BASE_DIR')  || define('BASE_DIR', str_replace(DS.'rails'.DS.'active_support'.DS.'utils'.DS.'scripts'.DS.'server.php','',__FILE__));

$_app_config_file = BASE_DIR.DS.'config'.DS.'config.php';

$public_dir = AkConfig::getDir('public');

include_once(CONTRIB_DIR.DS.'appserver-in-php'.DS.'autoload.php');
...
$app = new \MFS\AppServer\Middleware\URLMap\URLMap(array( '/' => function($context = null){
    ...
    //AkConfig::setOption('Request.remote_ip', REMOTE_IP);

    ob_start();
    $_headers = array('Server','Rails (via AppServer)');
...
echo "Rails dev server listening at 127.0.0.1:".$_SERVER['SERVER_PORT']."\n\n";
