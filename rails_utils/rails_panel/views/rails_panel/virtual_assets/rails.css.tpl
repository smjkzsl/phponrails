...
.flash.warning p span.icon{
   background:#ffdede  url('<%= url_for :action => "images", :id => "rails_panel_sprite", :format => "png" %>') no-repeat -4px -49px;
}

.flash.notice p span.icon{
    background:#fdf7d6  url('<%= url_for :action => "images", :id => "rails_panel_sprite", :format => "png" %>') no-repeat -4px -13px;
}
...
/* Main content for the Rails panel */
...
.tweets h3 span{
  ...
  display:block;
  float:left;
  background:transparent url('<%= url_for :action => "images", :id => "rails_panel_sprite", :format => "png" %>') no-repeat -4px -152px;
}
