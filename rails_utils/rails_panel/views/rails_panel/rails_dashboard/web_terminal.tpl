<div class="wide-content">
    <h1>_{Rails Terminal for <strong>%application_name</strong>}</h1>
    {?enabled}
    <p>_{The Rails terminal is running as user <strong>%user</strong>.}</p>
    {else}
    <%= flash_warning _("In order to use the Rails Terminal you need to define('ENABLE_TERMINAL_ON_DEV', true); on your \nconfig/environments/development.php file") %>
    {end}
</div>
...
function runUserCommand(command){
    ...
    if (user_command) {
        ...
        if(user_command == 'clear' || user_command == 'cls'){
            ...
            new Ajax.Request('<%= url_for 'action' => 'web_terminal', :controller => 'rails_dashboard' %>', {
...
<br />
{?enabled}
<%= flash_notice _("To disable the terminal, remove ENABLE_TERMINAL_ON_DEV from your config.php file") %>
{end}