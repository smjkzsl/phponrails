<div class="main-content main_content_left">
    {?new_install}
        <h1>_{It works!}</h1>
        <h2>_{Congratulations on your first Rails-powered page.}</h2>
        <p>_{Of course you haven't coded your app yet. Continue reading to learn what you need to do to create your Rails application.}</p>       
    {else}
        <h1>_{Rails Panel for %application_name}</h1>
    {end}
    ...
    <?php endif; ?>
        ...
        {?new_install}
            ...
            <div class="important-item-list quickstart">
                <ul>
                    ...
                    <%= format_snippet "./mrails generate", 'shell' %> 
                    _{and start coding}
                    </li>
                </ul>
            </div>
            ...
            <div class="important-item-list">
                <ul>
                    <li><%= link_to_guide _'Getting Started with Rails', 'getting_started' %></li>
                    <li><%= link_to_guide _'Rails Routing from the Outside In', 'routing' %></li>
                </ul>
            </div>
           ...
           <h2>_{Why I'm seeing this screen?}</h2>
            ...
            <p>_{You need to edit your config/routes.php file and disable or change the base path for the Rails Panel panel by editing the folowing route:}</p>
            <%= capture_snippet 'php' %>
        ...
        <div class="text-block radius_5">
            <dl>
                ...
                <dt>_{Host}</dt> <dd><?php echo HOST; ?></dd>
                ...
                <dt>_{Rails path}</dt> <dd>{rails_dir}</dd>
                <dt>_{Rails version}</dt> <dd><?php echo RAILS_VERSION; ?></dd>
                <dt>_{PHP version}</dt> <dd><?php echo PHP_VERSION; ?></dd>
                <dt>_{Memcached}</dt> <dd>{?memcached_on}_{On}{else}_{Off}{end}</dd>           
                <dt>_{Database}</dt><dd>{?database_settings-database}{database_settings-database}{else}_{none}{end}</dd>
        ...
        <h2>_{Who can access the Rails Panel?}</h2>
        <div class="text-block">
            ...
            <p>_{These are the IP addresses that can currently access the Rails Panel:} <strong><?php echo join(', ', $local_ips);?></strong></p>
        </div>
</div>
...
<div class="important-item-list">
<h3>_{PhpOnRails Framework}</h3>
<ul>
    <li><%= link_to _('API'), :controller => 'docs', :action => 'api', :module => 'rails_panel' %></li>
    <li><%= link_to _('Guides'), :controller => 'docs', :action => 'guide', :module => 'rails_panel' %></li>
</ul>
</div>
...
<div class="tweets">
    ...
    <?php $tweets = $rails_dashboard_helper->get_twitter_feeds(); ?>
    {loop tweets}
    <div class="tweet">
