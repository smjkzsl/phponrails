...
class DocsHelper extends AkBaseHelper
{
    ...
    public function render_doc($doc_contents){
        ...
        if($this->is_todo){
            $content = $this->t("WARNING: This guide has *NOT* been fully ported from Rails. Check how you can \"contribute to improve Rails\":/contribute\n").$content;
        }
    ...
    private function _setCodeBlocks($textile){
        if(preg_match_all('/<(yaml|shell|php|tpl|html|sql|plain)>(.*?)<\/\\1>/ms', $textile, $matches)){
            foreach ($matches[1] as $k => $class){
                ...
                $textile = str_replace($matches[0][$k], '<notextile>'.$this->_controller->rails_dashboard_helper->format_snippet($matches[2][$k], $css_class).'</notextile>', $textile);
            }
        }
        return $textile;
    }
