...
if(!empty($options['help'])){
    die(<<<HELP
Creating releases for Rails applications.
...
Valid options are:
    ...
    --app_name  Application name. (defaults to APP_NAME)
...
$default_app_name = (APP_NAME == 'Application') ? basename(MRAILS_BASE_DIR) : APP_NAME;
$is_rails_core = defined('CORE_DIR');
...
$options['app_name'] = AkInflector::underscore(empty($options['app_name']) ? $is_rails_core?'rails':$default_app_name:$options['app_name']);
...
$options['commit'] = isset($options['commit']) ? $is_rails_core : false;

$version_file = empty($options['version_file']) ? MRAILS_BASE_DIR.DS.'version.txt' : $options['version_file'];
if(!is_file($version_file)) $version_file = MRAILS_BASE_DIR.DS.'..'.DS.'version.txt';
...
$options['path'] = empty($options['path']) ? MRAILS_BASE_DIR.DS.'releases' : $options['path'];
strstr($options['path'], MRAILS_BASE_DIR) && AkFileSystem::make_dir($options['path'], array('base_path'=>MRAILS_BASE_DIR));
