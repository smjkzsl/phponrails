...
Example:
    rails ~/Code/PHP/weblog

    This generates a skeletal Rails installation in ~/Code/PHP/weblog.
    See the README in the newly created application to get going.

Usage: rails [-vsqhf] <-pd> 
    ...
    -v --version              Print the rails version.
    --base_dir                Print the path where rails resides.
...
class RailsAppInstaller
{
    ...
    public function guessEnvironment() {
        if(WIN){
            if(file_exists('D:/xampp/apache/conf/httpd.conf')){
    ...
    public function runDefaultXamppOnWindowsTasks() {
        ...
        $http_conf = file_get_contents('d:/xampp/apache/conf/httpd.conf');
        if(strstr($http_conf, '#LoadModule rewrite_module')){
            $this->yield('Enabling mod_rewrite');
            file_put_contents('d:/xampp/apache/conf/httpd.conf.rails', $http_conf);
            file_put_contents('d:/xampp/apache/conf/httpd.conf',
            str_replace(
            ...
            // Stop apache
            exec('D:\xampp\apache\bin\pv -f -k apache.exe -q');
            exec('rm D:\xampp\apache\logs\httpd.pid');

            // Start Apache in the background
            $shell = new COM('WScript.Shell');
            $shell->Run('D:\xampp\apache\bin\apache.exe', 0, false);
        }

        $my_cnf = @file_get_contents('D:/xampp/mysql/bin/my.cnf');
        ...
        if(strstr($my_cnf, '#innodb_')){
            ...
            file_put_contents('D:/xampp/mysql/bin/my.cnf.rails', $my_cnf);
            file_put_contents('D:/xampp/mysql/bin/my.cnf',
            str_replace(
            ...
            $shell = new COM('WScript.Shell');
            $shell->Run('D:\xampp\mysql\bin\mysqladmin --user=pma --password= shutdown', 0, false);
            $shell = new COM('WScript.Shell');
            $shell->Run('D:\xampp\mysql\bin\mysqld --defaults-file=D:\xampp\mysql\bin\my.cnf --standalone --console', 0, false);
        }
    }
    ...
    protected function _copyRailsToVendors() {
        $this->yield("\nCopying rails framework from ".RAILS_DIR." to ".$this->options['directory'].DS.'vendor'.DS.'rails');

        self::copyRecursivelly(RAILS_DIR, $this->options['directory'].DS.'vendor'.DS.'rails');
    }

    protected function _copyApplicationFiles($directory_structure, $base_path = '.') {
        foreach ($directory_structure as $k=>$node){

            $path = $base_path.DS.(is_array($node) ? serialize($node) : $node);
            if(is_dir($path)){
                $this->_makeDir($path);
            }elseif(is_file($path)){
                $this->_copyFile($path);
    ...
    protected function _cleanUpAndCreateEmptyFolders(){
        ...
        // Copy docs
        self::copyRecursivelly($this->options['directory'].DS.'vendor'.DS.'rails'.DS.'docs', $this->options['directory'].DS.'docs');
    ...
    static function getAbsolutePath($path) {
        $_path = $path;
        if (!preg_match((WIN ? "/^\w+:/" : "/^\//"), $path )) {
            $current_dir = WIN ? str_replace("\\", DS, realpath('.').DS) : realpath('.').DS;
...
$directory   = RailsAppInstaller::getAbsolutePath(
...
$public_html = empty($public_html) ? false : RailsAppInstaller::getAbsolutePath($public_html);
...
if($prompt){
    echo "\nCreate an Rails ".RAILS_VERSION." application in $directory\n";
...
$Installer = new RailsAppInstaller(array(
'directory'     =>$directory,
