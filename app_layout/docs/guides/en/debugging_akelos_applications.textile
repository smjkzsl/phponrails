...
This guide introduces techniques for debugging Rails applications. By referring to this guide, you will be able to:
...
To enable it, add the following in the +Rails::Initializer do |config|+ block inside +environment.php+:
...
Rails makes use of PHP's standard +logger+ to write log information. You can also substitute another logger such as +Log4R+ if you wish.
...
TIP: By default, each log is created under +BASE_DIR/log/+ and the log file name is +environment_name.log+.
...
The debugger used by Rails, +php-debug+, comes as a gem. To install it, just run:
...
Rails has had built-in support for php-debug since Rails 2.0. Inside any Rails application you can invoke the debugger by calling the +debugger+ method.
...
<shell>
~/PathTo/rails_project$ ./mrails server --debugger
...
(rdb:5) where
    ...
    #1 Kernel.s}
       at line /PathTo/project/vendor/rails/actionpack/lib/action_controller/base.php:1175
    #2 AkActionController.perform_action_without_filters
       at line /PathTo/project/vendor/rails/actionpack/lib/action_controller/base.php:1175
    #3 ActionController::Filters::InstanceMethods.call_filters(chain#ActionController::Fil...,...)
       at line /PathTo/project/vendor/rails/actionpack/lib/action_controller/filters.php:617
...
</shell>
...
If you want to go deeper into the stack trace you can move single +steps+, through your calling methods and into Rails code. This is one of the best ways to find bugs in your code, or perhaps in PHP or Rails.
...
<shell>
(rdb:5) break 10
Breakpoint 1 file /PathTo/project/vendor/rails/actionpack/lib/action_controller/filters.php, line 10
</shell>
...
A PHP application (on Rails or not), can leak memory - either in the PHP code or at the C code level.
...
<php>
require 'bleak_house' if ENV['BLEHOUSE']
</php>

Start a server instance with BleakHouse integration:

<shell>
RAILS_ENV=production BLEHOUSE=1 php-bleak-house ./script/server
</shell>
...
* "Exception Notifier":http://github.com/rails/exception_notification/tree/master: Provides a mailer object and a default set of templates for sending email notifications when errors occur in an Rails application.
...
* "Article: Debugging an Rails application with php-debug":http://www.sitepoint.com/article/debug-rails-app-php-debug/
* "php-debug Basics screencast":http://brian.maybeyoureinsane.net/blog/2007/05/07/php-debug-basics-screencast/
* "Ryan Bate's php-debug screencast":http://railscasts.com/episodes/54-debugging-with-php-debug
...
* "Rails Wiki: How to Configure Logging":http://wiki.rails.org/rails/pages/HowtoConfigureLogging
