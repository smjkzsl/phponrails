...
This guide won't teach you to write an Rails application; it assumes basic familiarity with the Rails way of doing things.
...
When you do end up destroying your testing database (and it will happen, trust me), you can rebuild it from scratch according to the specs defined in the development database. You can do this by running +./mrails db:test:prepare+.

h4. Rails Sets up for Testing from the Word Go

Rails creates a +test+ folder for you as soon as you create an Rails project using +rails _application_name_+. If you list the contents of this folder then you shall see:
...
You'll find fixtures under your +test/fixtures+ directory. When you run +./mrails generate model+ to create a new model, fixture stubs will be automatically created and placed in this directory.
...
tag is considered PHP code. When this fixture is loaded, the +size+ attribute of the three records will be set to 20/50, 20/2, and 20-69 respectively. The +brightest_on+ attribute will also be evaluated and formatted by Rails to be compatible with the database.
...
When you use +./mrails generate scaffold+, for a resource among other things it creates a test stub in the +test/unit+ folder:

<pre>
$ ./mrails generate scaffold post title:string body:text
...
...
Before you can run your tests you need to ensure that the test database structure is current. For this you can use the following mrails commands:
...
Above +./mrails db:migrate+ runs any pending migrations on the _development_ environment and updates +db/schema.php+. +./mrails db:test:load+ recreates the test database from the current db/schema.php. On subsequent attempts it is a good to first run +db:test:prepare+ as it first checks for pending migrations and warns you appropriately.

NOTE: +db:test:prepare+ will fail with an error if db/schema.php doesn't exists.

h5. Mrails Tasks for Preparing your Application for Testing

|_.Tasks                         |_.Description|
|+./mrails db:test:clone+            |Recreate the test database from the current environment's database schema|
|+./mrails db:test:clone_structure+  |Recreate the test databases from the development structure|
|+./mrails db:test:load+             |Recreate the test database from the current +schema.php+|
|+./mrails db:test:prepare+          |Check for pending migrations and load the test schema|
|+./mrails db:test:purge+            |Empty the test database.|

TIP: You can see all these mrails tasks and their descriptions by running +./mrails --tasks --describe+
...
TIP: Many Rails developers practice _Test-Driven Development_ (TDD). This is an excellent way to build up a test suite that exercises every part of your application. TDD is beyond the scope of this guide, but one place to start is with "15 TDD steps to create an Rails application":http://andrzejonsoftware.blogspot.com/2007/05/15-tdd-steps-to-create-rails.html.
...
|+assertRecognizes(expected_options, path, extras={}, message=nil)+                |Asserts that the routing of the given path was handled correctly and that the parsed options (given in the expected_options array) match path. Basically, it asserts that Rails recognizes the route given by expected_options.|
...
Now that we have used Rails scaffold generator for our +Post+ resource, it has already created the controller code and functional tests. You can take look at the file +posts_controller_test.php+ in the +test/functional/controllers+ directory.
...
The +assert_select+ assertion is quite powerful. For more advanced usage, refer to its "documentation":http://api.rails.org/classes/ActionController/Assertions/SelectorAssertions.html.
...
<shell>
$ ./mrails generate integration_test user_flows
...
h3. Mrails Tasks for Running your Tests

You don't need to set up and run your tests by hand on a test-by-test basis. Rails comes with a number of mrails tasks to help in testing. The table below lists all mrails tasks that come along in the default makefile.php when you initiate an Rails project.

|_.Tasks                         |_.Description|
|+./mrails test+                     |Runs all unit, functional and integration tests. You can also simply run +./mrails+ as the _test_ target is the default.|
|+./mrails test:units+               |Runs all the unit tests from +test/unit+|
|+./mrails test:functionals+         |Runs all the functional tests from +test/functional+|
|+./mrails test:integration+         |Runs all the integration tests from +test/integration+|
|+./mrails test:recent+              |Tests recent changes|
|+./mrails test:uncommitted+         |Runs all the tests which are uncommitted. Only supports Subversion|
|+./mrails test:plugins+             |Run all the plugin tests from +vendor/plugins/*/**/test+ (or specify with +PLUGIN=_name_+)|
