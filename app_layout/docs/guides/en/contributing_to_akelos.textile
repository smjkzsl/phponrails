...
Query caching is an Rails feature that caches the result set returned by each query so that if Rails encounters the same query again for that request, it will use the cached result set as opposed to running the query against the database again.
...
Rails 2.1 and above provide +ActiveSupport::Cache::Store+ which can be used to cache strings. Some cache store implementations, like +MemoryStore+, are able to cache arbitrary PHP objects, but don't count on every cache store to be able to do that.

The default cache stores provided with Rails include:

1) +ActiveSupport::Cache::MemoryStore+: A cache store implementation which stores everything into memory in the same process. If you're running multiple Rails server processes (which is the case if you're using mongrel_cluster or Phusion Passenger), then this means that your Rails server process instances won't be able to share cache data with each other. If your application never performs manual cache item expiry (e.g. when you‘re using generational cache keys), then using +MemoryStore+ is ok. Otherwise, consider carefully whether you should be using this cache store.  
...
NOTE: +config.cache_store+ can be used in place of +AkActionController.cache_store+ in your +Rails::Initializer.run+ block in +environment.php+

In addition to all of this, Rails also adds the +ActiveRecord#cache_key+ method that generates a key using the class name, +id+ and +updated_at+ timestamp (if available).
...
* "RailsEnvy, Rails Caching Tutorial, Part 2":http://www.railsenvy.com/2007/3/20/php-on-rails-caching-tutorial-part-2
* "ActiveSupport::Cache documentation":http://api.rails.org/classes/ActiveSupport/Cache.html
