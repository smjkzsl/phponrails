...
class AkDebug
{
    ...
    */
    static function trace($text = null, $line = null, $file = null, $method = null, $escape_html_entities = true) {
        ...
        if(CLI){
            ...
            foreach ($methods as $method){
                ...
                if(defined('TRACE_DUMP_METHOD')){
                    if(TRACE_DUMP_METHOD == $method){
                        $pre_style = '';
                    }
                ...
                $formatted .= "<div style='margin:10px;'><a href='javascript:void(0);' onclick='e_$element_id = document.getElementById(\"$element_id\"); e_$element_id.style.display = (e_$element_id.style.display == \"none\"?\"block\":\"none\");' title='Set the constant TRACE_DUMP_METHOD to your favourite default method'>$method</a><br />".
    ...
    static function dump($var, $method = null, $max_length = null) {
        $method = empty($method) ? (defined('TRACE_DUMP_METHOD') ? TRACE_DUMP_METHOD : 'var_dump') : $method;
        ...
        $max_length = defined('DUMP_MAX_LENGTH') ? DUMP_MAX_LENGTH : 10000000;
        $result = $max_length ? substr($contents, 0, $max_length) : $contents;
        if($contents != $result){
            $result .= ' ...dump truncated at max length of '.$max_length.' chars define DUMP_MAX_LENGTH to false or to a larger number';
        }
        ob_end_clean();
        return $result;
    }
        ...
        if(!$only_app){
            ...
            for($i = $start_level-1; $i <= count($backtrace) - 1; $i++){
                if(isset($backtrace[$i]["line"])){
                    if(strstr($backtrace[$i]["file"], COMPILED_VIEWS_DIR) || strstr($backtrace[$i]["file"], AkConfig::getDir('app'))){
    ...
    */
     ...
     *     }
     *
     * IMPORTANT NOTE: You must define ENABLE_PROFILER to true for this to work.
    */
