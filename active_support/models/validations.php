...
class AkModelValidations extends AkModelExtenssion
{
    ...
    public function validatesUniquenessOf($attribute_names, $options = array()) {
        ...
        foreach ((array)$attribute_names as $attribute_name){
            ...
            if(is_null($value) || ($options['case_sensitive'] || !$this->_Model->hasColumn($attribute_name))){
                ...
                include_once(CONTRIB_DIR.DS.'phputf8'.DS.'utf8.php');
                if($is_active_record){
    ...
    *
    * There are some regular expressions bundled with the PhpOnRails Framework.
    * You can override them by defining them as PHP constants (Ie. define('EMAIL_REGULAR_EXPRESSION', '/^My custom email regex$/');). This must be done on your main configuration file.
    * This are predefined perl-like regular extensions.
    *
    * * NOT_EMPTY_REGULAR_EXPRESSION ---> /.+/
    * * EMAIL_REGULAR_EXPRESSION ---> /^([a-z0-9_\-\.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([a-z0-9\-]+\.)+))([a-z]{2,4}|[0-9]{1,3})(\]?)$/i
    * * NUMBER_REGULAR_EXPRESSION ---> /^[0-9]+$/
    * * PHONE_REGULAR_EXPRESSION ---> /^([\+]?[(]?[\+]?[ ]?[0-9]{2,3}[)]?[ ]?)?[0-9 ()\-]{4,25}$/
    * * DATE_REGULAR_EXPRESSION ---> /^(([0-9]{1,2}(\-|\/|\.| )[0-9]{1,2}(\-|\/|\.| )[0-9]{2,4})|([0-9]{2,4}(\-|\/|\.| )[0-9]{1,2}(\-|\/|\.| )[0-9]{1,2})){1}$/
    * * IP4_REGULAR_EXPRESSION ---> /^((25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]|[0-9])\.){3}(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]|[0-9])$/
    * * POST_CODE_REGULAR_EXPRESSION ---> /^[0-9A-Za-z  -]{2,7}$/
    *
