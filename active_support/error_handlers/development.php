...
if(defined('DEBUG') && DEBUG){
    ...
    function ak_development_error_handler($error_number, $error_message, $file, $line) {
        ...
        $result = DEBUG_OUTPUT_AS_HTML ? '<pre>' : '';
        ...
        $result .= !DEBUG_OUTPUT_AS_HTML ? html_entity_decode(strip_tags($result)) : '<div style="background-color:#fff;margin:10px;padding:10px;color:#000;font-family:sans-serif;border-bottom:3px solid #f00;font-size:12px;">'. $result.'</div>';
    ...
    !defined('TRACE_ONLY_APP_DB_QUERIES') && define('TRACE_ONLY_APP_DB_QUERIES', true);
    !defined('TRACE_DB_QUERIES_INCLUDES_DB_TYPE') && define('TRACE_DB_QUERIES_INCLUDES_DB_TYPE', false);

    function ak_trace_db_query($message, $new_line = true) {
        if(Ak::getStaticVar('ak_trace_db_query') === false){
            return ;
        }
        if(!TRACE_DB_QUERIES_INCLUDES_DB_TYPE){
            $message = preg_replace('/\([a-z0-9]+\): /','', trim($message, "\n-"));
        }
        $details = AkDebug::getLastFileAndLineAndMethod(TRACE_ONLY_APP_DB_QUERIES);
        if(empty($details)){
