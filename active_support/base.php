...
/**
* PhpOnRails Framework static functions
*
* Ak contains all the PhpOnRails Framework static functions. This
...
class Ak
{
    ...
    */
    ...
    static function deprecateWarning($message, $fatal=false) {
        if (!$fatal && ENVIRONMENT == 'production'){
            return;
        }
    ...
    */
	...
	static function m_implode($ch, &$ar) {
	  foreach($ar as &$v) {
	    if(is_array($v)) $v = Ak::m_implode($ch, $v);
	  }
    ...
    */
    static function t($string, $args = null, $controller = null) {
        ...
        if(AUTOMATICALLY_UPDATE_LANGUAGE_FILES && ($locale_manager_class == false || $locale_manager_class == 'AkLocaleManager')) {
            if(!$_custom_dev_shutdown && defined('LOCALE_MANAGER') && class_exists(LOCALE_MANAGER) && in_array('AkLocaleManager',class_parents(LOCALE_MANAGER))) {
                $locale_manager_class = LOCALE_MANAGER;
        ...
        if((AUTOMATICALLY_UPDATE_LANGUAGE_FILES || (defined('TEST_TRANSLATIONS') && TEST_TRANSLATIONS)) && !empty($string) && is_string($string)){
            ...
            if($_dev_shutdown && (!defined('TEST_TRANSLATIONS') || !TEST_TRANSLATIONS)){
                register_shutdown_function(array($locale_manager_class,'updateLocaleFiles'));
                $_dev_shutdown = false;
            }
        }
        ...
        if(!isset($lang)){
            ...
            if(is_array(@$dictionary[0])){
                foreach($dictionary as $k=>$v)
                    $framework_dictionary = array_merge((array)$framework_dictionary,(array)$v);
            }else{
        ...
        if(!empty($string) && is_array($string)){
            if(!empty($string[$lang])){
                if(defined('TRANSLATION_DEBUG') && TRANSLATION_DEBUG){
        ...
        /**
        * @todo Prepare for multiple locales by inspecting DEFAULT_LOCALE
        */
        if(defined('TRANSLATION_DEBUG') && TRANSLATION_DEBUG){
    ...
    */
    static function translate($string, $target_language, $namespace = false) {
        $from = is_array($target_language) ? $target_language[0] : 'cn' ;
    ...
    */
    ...
    static function locale($locale_setting, $locale = null) {
        ...
        $locale = empty($locale) ? (defined('LOCALE') ? LOCALE : (Ak::t('Rails') && Ak::locale($locale_setting))) : $locale;
        ...
        if (empty($settings[$locale])) {
            if(func_num_args() != 3){ // First time we ask for something using this locale so we will load locale details
                ...
                $p=AkConfig::getDir('config').DS.'locales'.DS.Ak::sanitize_include($requested_locale,'high').'.php';
                 //~ echo $p."\n";
                if(@include($p)){
                ...
                if(isset($settings[$locale]['charset'])){
                    defined('CHARSET') ? null : (define('CHARSET',$settings[$locale]['charset']) && @ini_set('default_charset', CHARSET));
                }
            }
        }
    ...
    static function lang($set_language = null) {
        static $lang;
        $lang = empty($set_language) ? (empty($lang) ? FRAMEWORK_LANGUAGE : $lang) : $set_language;
        return $lang;
    }
    ...
    static function langs() {
        ...
        if(defined('APP_LOCALES')){
            $langs = array_diff(explode(',',APP_LOCALES.','),array(''));
        }
    ...
    */
    ...
    static function &getLogger($namespace = ENVIRONMENT) {
        static $Logger = array();
    ...
    */
    static function time() {
        return time()+(defined('TIME_DIFERENCE') ? TIME_DIFERENCE*3600 : 0);
    }

    static function gmt_time() {
        return Ak::time()+(TIME_DIFERENCE_FROM_GMT*3600);
    }
        ...
        if (preg_match("|^(([0-9]{1,2}):?([0-9]{1,2}):?([0-9\.]{1,4}))?|", ($iso_date_or_hour), $rr)){
            return empty($rr[0]) ? Ak::time() : mktime($rr[2],$rr[3],$rr[4]);
                ...
                if($rr[1]>=2038 || $rr[1]<=1970){
                    require_once(CONTRIB_DIR.DS.'adodb'.DS.'adodb-time.inc.php');
    ...
    *   SMTP_SERVER, SMTP_USER and SMTP_PASSWORD, respectively.
    *
    *
    *   @param $from
    *
    ...
    */
    static function mail ($from, $to, $subject, $body, $additional_headers = array()) {
        require_once(CONTRIB_DIR.DS.'pear'.DS.'Mail.php');
    ...
    */
    ...
    static function encrypt($data, $key = null) {
        $key = empty($key) ? md5(SESSION_NAME) : $key;
    ...
    static function blowfishEncrypt($data, $key = null) {
        $key = empty($key) ? md5(SESSION_NAME) : $key;
        $key = substr($key,0,56);
        require_once(CONTRIB_DIR.DS.'pear'.DS.'Crypt'.DS.'Blowfish.php');
    ...
    static function compress($data, $format = 'gzip') {
        ...
        $compressed_file = TMP_DIR.DS.'d'.$key;
        $uncompressed_file = TMP_DIR.DS.'s'.$key;
        if(AkFileSystem::file_put_contents($uncompressed_file, $data, array('base_path'=>TMP_DIR)) !== false){
            ...
            trigger_error(Ak::t('Could not write to temporary directory for generating compressed file using Ak::compress(). Please provide write access to %dirname', array('%dirname'=>TMP_DIR)), E_USER_ERROR);
        }
        $result = AkFileSystem::file_get_contents($compressed_file, array('base_path'=>TMP_DIR));
        AkFileSystem::file_delete($compressed_file, array('base_path'=>TMP_DIR));
        AkFileSystem::file_delete($uncompressed_file, array('base_path'=>TMP_DIR));
        return $result;
    }

    static function uncompress($compressed_data, $format = 'gzip') {
        $key = Ak::randomString(15);
        $compressed_file = TMP_DIR.DS.'s'.$key;
        $uncompressed_file = TMP_DIR.DS.'d'.$key;

        if(AkFileSystem::file_put_contents($compressed_file, $compressed_data, array('base_path'=>TMP_DIR)) !== false){
            ...
            trigger_error(Ak::t('Could not write to temporary directory for generating uncompressing file using Ak::uncompress(). Please provide write access to %dirname', array('%dirname'=>TMP_DIR)), E_USER_ERROR);
        }
        $result = AkFileSystem::file_get_contents($uncompressed_file, array('base_path'=>TMP_DIR));
        AkFileSystem::file_delete($uncompressed_file, array('base_path'=>TMP_DIR));
        AkFileSystem::file_delete($compressed_file, array('base_path'=>TMP_DIR));
        return $result;
    }
    ...
    /**
    * The PhpOnRails Framework has an standardized way to convert between formats.
    ...
    */
    static function convert() {
        ...
        $options['path'] = rtrim(empty($options['path']) ? ACTIVE_SUPPORT_DIR.DS.'converters' : $options['path'], DS."\t ");
        ...
        if(!class_exists($converter_class_name)){
            ...
            if(!file_exists($file_name)){
                if(defined('REMOTE_CONVERTER_URI')){
    ...
    static function &call_user_func_array($function_name, $parameters) {
        Ak::deprecateWarning('Ak::call_user_func_array() is deprecated and will be removed from Rails in future releases. Please use PHP\'s native call_user_func_array() function instead.');
    ...
    static function mime_content_type($file) {
        static $mime_types;
        empty($mime_types) ? include ACTIVE_SUPPORT_DIR.DS.'utils'.DS.'mime_types.php' : null;
    ...
    */
    ...
    static function &_staticVar($name, &$value = null, $destruct = false) {
        static $_memory;
        if(!constant('CAN_FORK') || (!$pid = getmypid())){
            $pid = 0;
        }
     ...
     */
    static function getSettings($namespace, $raise_error_if_config_file_not_found = true, $environment = ENVIRONMENT) {
        static $_config;
        if ($raise_error_if_config_file_not_found && !in_array($environment,Ak::toArray(AVAILABLE_ENVIRONMENTS))) {
            trigger_error('The environment '.$environment.' is not allowed. Allowed environments: '.AVAILABLE_ENVIRONMENTS, E_USER_ERROR);
            return false;
        }
        if (!isset($_config)) {
     ...
     *
     * On Rails we can do Ak::get('Model')->find();
     */
    ...
    */
    ...
    /**
     * Rails version of pcntl_fork which prevents forked processes from killing the database connection.
     *
    ...
    */
    static function get_tmp_dir_name() {
        if(!defined('TMP_DIR')){
            if(defined('BASE_DIR')){
                $tmp_dir = BASE_DIR.DS.'tmp'.DS.ENVIRONMENT.DS.(CLI?'console':'web');
            ...
            if(empty($dir)){
                trigger_error('Could not find a path for temporary files. Please define TMP_DIR in your config.php', E_USER_ERROR);
            }
            $dir = rtrim(realpath($dir), DS).DS.'ak_'.md5(BASE_DIR).DS.(WEB_REQUEST ? 'web' : 'cli');
            if(!is_dir($dir)){
                mkdir($dir, 0777, true);
            }
            return $dir;
    ...
    static function registerAutoloader($autoloader) {
        spl_autoload_unregister('rails_autoload');
    ...
    if(!isset($ran_tests[$test_case_name]) || !$prevent_double_test_running){
        if(!defined('ALL_TESTS_CALL')){
            ...
            if(empty($custom_reporter)){
                if (defined('CLI') && CLI || TextReporter::inCli() || (defined('CONSOLE_MODE') && CONSOLE_MODE) || (defined('WEB_REQUEST') && !WEB_REQUEST)) {
...
function ak_test_case($test_case_name, $show_enviroment_flags = true)
{
    ...
    if ($levels == 1 || ($levels == 2 && isset($_ENV['SCRIPT_NAME']) && $_ENV['SCRIPT_NAME'] == 'dummy.php')) {
        if($show_enviroment_flags){
            echo "(".ENVIRONMENT." environment) Error reporting set to: ".AkConfig::getErrorReportingLevelDescription()."\n";
        }
...
function ak_compat($function_name)
{
    if(!function_exists($function_name)){
        require_once(CONTRIB_DIR.DS.'pear'.DS.'PHP'.DS.'Compat'.DS.'Function'.DS.$function_name.'.php');
    }
}
...
function ak_define($name, $value = null)
{
    ...
    $name = substr($name,0,3) == '' ? $name : ''.$name;
