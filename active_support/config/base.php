 ...
 *
 * APP_DIR/config/myconfig.yml
 *
 * can be accessed via:
 *
 *      $config = new AkConfig();
 *      $config->get('myconfig'); // loads myconfig.yml and section "ENVIRONMENT"
 *
 * By default the configuration for the environment defined in ENVIRONMENT will be loaded.
 *
...
class AkConfig
{
    ...
    public function &get($namespace, $environment = ENVIRONMENT, $raise_error_if_config_file_not_found = true) {
        //~ echo $namespace;
        $key = $this->_getCacheKey($namespace,$environment);
        if(empty($this->options['skip_cache']) && ($config = Ak::getStaticVar($key))){
            return $config;
        }
        if (IN_SAE || !($config = $this->readCache($namespace, $environment))) {
    ...
    static function getLocalesReady() {
        Ak::t('Rails');
        ENABLE_PROFILER &&  AkDebug::profile('Got multilingual ');
    }

    static function parseSettingsConstants($settingsStr) {
        return preg_replace_callback('/\$\{(.*?)\}/', create_function('$arr', 'return defined($arr[1]) ? constant($arr[1]) : "";'), $settingsStr);
    }

    public function readConfig($namespace, $environment = ENVIRONMENT, $raise_error_if_config_file_not_found = true) {
    ...
    public function readConfigYaml($namespace, $yaml_string, $environment = ENVIRONMENT){
    ...
    public function readConfigArray($namespace, $config = array(), $environment = ENVIRONMENT){
        if (!is_array($config)){
            return false;
        }
        if(!isset($config['default']))
            return $config; //caching 无需设置 环境此项
        $default = isset($config['default']) ? $config['default'] : array();
        $configs = array();
        unset($config['default']);
        $environments = array_keys($config);
        $default_environments = Ak::toArray(AVAILABLE_ENVIRONMENTS);
    ...
    static function getCacheFileName($namespace, $environment = ENVIRONMENT) {
    ...
    public function readCache($namespace, $environment = ENVIRONMENT, $force = false) {
    ...
    public function writeCache($config, $namespace, $environment = ENVIRONMENT, $force = false) {
...
CACHE;
    ...
    public function clearStaticCache($namespace, $environment = ENVIRONMENT){
        $key = $this->_getCacheKey($namespace,$environment);
    ...
    protected function _useReadCache($environment = ENVIRONMENT) {
        if(CLI && ENVIRONMENT != 'testing'){
            return false;
        }
        return ($environment == 'production' || $environment == 'setup');
    }

    protected function _useWriteCache($environment = ENVIRONMENT) {
        if(CLI && ENVIRONMENT != 'testing'){
            return false;
        }
    ...
    protected function _getCacheKey($namespace, $environment){
        return '_config_'.$namespace.$environment.WEB_REQUEST;
    }
}
