...
class AkTestRequest extends AkRequest
{
    public function &recognize($Map = null) {
        ...
        if(!is_file($controller_path) || !include_once($controller_path)){
            LOG_EVENTS && Ak::getLogger()->error('Controller '.$controller_path.' not found.');
        ...
        if(!class_exists($controller_class_name)){
            LOG_EVENTS && Ak::getLogger()->error('Controller '.$controller_path.' does not implement '.$controller_class_name.' class.');
            if(ENVIRONMENT == 'development'){
