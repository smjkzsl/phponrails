...
class AkRoute
{
    ...
    protected function assembleUrlFromSegments(&$params) {
        ...
        foreach (array_reverse($this->getSegments()) as $name => $segment){
            //if(isset($params[$name]) && $params[$name]!=$this->segments[$name])
             //   throw new RouteDoesNotMatchParametersException("路由不匹�?$name is not matched.");
	     //~ try{
	     $VV=@$params[$name];
	     if (!$url_piece = $segment->generateUrlFromValue($VV, $omit_defaults)) continue;
	     //~ if($VV && $VV!=$segment ->default)
		//~ }catch(Exception $e){
			//~ throw new RouteDoesNotMatchParametersException("路由不匹�?$name is not matched.");
		//~ }
    ...
    protected function addFormatToParams(&$params) {
        if(!AUTOMATICALLY_ACCEPT_KNOW_FORMATS || empty($params) || isset($params['format'])) return;
    ...
    protected function buildQueryStringFor($params) {
		...
		//~ AkDebug::trace($this->defaults[$name].' <> '.$value);
    ...
    */
    ...
    private function _rebaseApplicationIfRequired() {
        if(isset($this->requirements['rebase'])){
            AkConfig::setOption('rebase_path', (WIN?'':DS).$this->requirements['rebase']);
