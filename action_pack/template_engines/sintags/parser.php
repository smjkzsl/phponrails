...
{
    public $_SINTAGS_OPEN_HELPER_TAG = SINTAGS_OPEN_HELPER_TAG;
    public $_SINTAGS_CLOSE_HELPER_TAG = SINTAGS_CLOSE_HELPER_TAG;
    public $_SINTAGS_HASH_KEY_VALUE_DELIMITER = SINTAGS_HASH_KEY_VALUE_DELIMITER;
    ...
    public function PhpCode($match, $state) {
        if(!SINTAGS_REPLACE_SHORTHAND_PHP_TAGS){
            $this->output .= $match;
            return true;
        }
        switch ($state){
    ...
    private function _getAvailableHelpers() {
        ...
        if(empty($this->available_helpers)){
            if(defined('SINTAGS_AVAILABLE_HELPERS')){
                $helpers = unserialize(SINTAGS_AVAILABLE_HELPERS);
