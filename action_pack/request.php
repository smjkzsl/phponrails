...
class AkRequest
{
    ...
    * String parse method.
    *
    * This method gets a petition as parameter, using the "Rails" request 
    ...
    */
    ...
    public function getLocaleFromUrl() {
        ...
        if(strstr($this->getUrl(),SITE_URL.$locale)){
            return $locale;
        }
        return '';
    }
    ...
    * a different <tt>tld_length</tt>, such as 2 to catch rails.co.uk in 'www.rails.co.uk'.
    */
    ...
    * You can specify a different <tt>tld_length</tt>, such as 2 to catch ['www'] instead of ['www', 'rails']
    * in 'www.rails.co.uk'.
    */
    ...
    */
    public function getPort() {
        $this->port_as_int = WEB_REQUEST ? SERVER_PORT : 80;
    ...
    */
    ...
    public function getHost() {
        ...
        return WEB_REQUEST ? $this->env['SERVER_NAME'] : 'localhost';
    }
    ...
    */
    ...
    static function isLocal(){
        return in_array(AkConfig::getOption('Request.remote_ip', REMOTE_IP), AkConfig::getOption('local_ips', array('localhost','127.0.0.1','::1')));
    }
    ...
    */
    public function &recognize($Map = null) {
        ...
        if($this->mapRoutes($Map)){
            if(LOG_EVENTS) Ak::getLogger()->info('Processing '.$this->getController().'#'.$this->getAction().' (for '.$this->getRemoteIp().')');
    ...
    public function reportError($options = array()){
        if(LOG_EVENTS && !empty($options['log'])) Ak::getLogger()->error($options['log']);

        if(DEV_MODE && !empty($options['message'])){
