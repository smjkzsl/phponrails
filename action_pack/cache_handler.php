...
/**
 * Rails supports three types of caching:
 *
 * - Page Caching
 * - Action Caching
 * - Fragment Caching
 ...
 *
 * If no cache directory is set, the Cache_Lite will use the TMP_DIR constant value for the
...
class AkCacheHandler
{
    ...
    */
    public function expirePage($path = null, $language=null) {
        ...
        if(IN_SAE && $this->_settings['handler']['type']==3){
            $cacheId = TMP_DIR.DS.'cache'.DS.$cacheGroup.DS.$cacheId.'.php';
        }
    ...
    protected function _storePageCache($content, $cacheId,$cacheGroup) {
        $filename = TMP_DIR.DS.'cache'.DS.$cacheGroup.DS.$cacheId.'.php';
        if(IN_SAE && $this->_settings['handler']['type']==3){
            $this->_cache_store->save($content,$filename,$cacheGroup);
            return $filename;
        }
...
EOF;
    ...
    protected function _setCachesPage($options) {
        ...
        'headers'=> array('X-Cached-By'=>'Rails'));
        Ak::parseOptions($options, $default_options,array(),true);
    ...
    protected function _buildCacheId($path, $forcedLanguage = null, $normalize = true) {
        ...
        $cacheId = ($forcedLanguage!=null?$forcedLanguage:$this->_getDefaultLanguageForUser()).'/'. $cacheId;
    ...
    protected function _buildCacheGroup($options = array()) {
        ...
        $this->_lastCacheGroup = $this->_convertGroup(isset($_SERVER['HOST']) ? $_SERVER['HOST'] : HOST);
    ...
    */
    public function beforeActionCache() {
        ...
        if (($content = $this->readFragment($this->_action_cache_path, $options))!==false) {
            ...
            $format = $this->_controller->Request->getFormat();
            $this->_controller->Response->addHeader('X-Cached-By','Rails-Action-Cache');
    ...
    protected function _pathFor($options = array(), $normalize = true) {
        ...
        if ($normalize && (!isset($options['action']) || (isset($options['action']) && $options['action']==DEFAULT_ACTION && !strstr($path,'/'.DEFAULT_ACTION)))) {
            $path = rtrim($path,'/');
    ...
    static public function sendAdditionalHeaders($sendHeaders = true, $returnHeaders = false) {
        $additionalHeaders = array('X-Cached-By: Rails');
        if ($sendHeaders) {
