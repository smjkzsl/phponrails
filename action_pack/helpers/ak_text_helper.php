...
defined('VALID_URL_CHARS_REGEX') || define('VALID_URL_CHARS_REGEX','A-Z-a-z0-9:=?&\/\.\-\\%~#_;,+');
defined('AUTO_LINK_REGEX')       || define('AUTO_LINK_REGEX','/
        (                          # leading text
...
* the level of inline RailsFramework code in the templates. In the example
...
class AkTextHelper extends AkBaseHelper
{
    ...
    */
    static function truncate($text, $length = 30, $truncate_string = '...', $break = false) {
        require_once(CONTRIB_DIR.DS.'phputf8'.DS.'utf8.php');
	...
	*/
    ...
    static function textilize($text, $options = array()) {
        ...
        $options = array_merge($default_options, $options);
        require_once(CONTRIB_DIR.DS.'TextParsers'.DS.'Textile.php');
        if (!empty($text)) {
            $Textile = new Textile();
    ...
    * Example:
    *
    *   <?=$text_helper->auto_link("Go to http://www.rails.org and say hello to bermi@example.com");?>
    *   //outputs: Go to <a href="http://www.rails.org">http://www.rails.org</a> and
	...
	*/
    ...
    static function auto_link_urls($text, $href_options = array()) {
        ...
        return preg_replace_callback(AUTO_LINK_REGEX, create_function(
        '$matched',
    ...
    static function get_urls_from_text($text) {
        $urls = array();
        if(preg_match_all(AUTO_LINK_REGEX, $text, $found_urls)){
     ...
     *
     *  Example:
     * $text_helper->get_linked_urls_from_text('<a href="http://bermilabs.com">Rails.com</a>');
     * //returns: array('http://bermilabs.com');
     */
    static function get_linked_urls_from_text($text) {
        $linked_urls = array();
        if(preg_match_all('/<a [^>]*href[ ]?=[ \'"]?(['.VALID_URL_CHARS_REGEX.']+)[ \'"]?[^>]*>/i', $text, $linked_urls_pieces)){
    ...
    static function get_image_urls_from_html($text) {
        $linked_urls = array();
        if(preg_match_all('/<img[^>]+src[ ]?=[ \'"]?(['.VALID_URL_CHARS_REGEX.']+)[ \'"]?[^>]*>/is', $text, $linked_urls_pieces)){
            $linked_urls = array_unique($linked_urls_pieces[1]);
        }
        return $linked_urls;
    }
    ...
    */
    static function markdown($text) {
        if (!empty($text)) {
            require_once(CONTRIB_DIR.DS.'TextParsers'.DS.'markdown.php');
