 ...
 * set of forbidden functionalities extend this class and set PHP_CODE_SANITIZER_FOR_TEMPLATE_HANDLER
...
class AkPhpCodeSanitizer
{
    ...
    public function isCodeSecure($code =null, $raise_if_insecure = true) {
        ...
        if(!defined('PHP_CODE_SANITIZER_SKIP_VARIABLES') || !PHP_CODE_SANITIZER_SKIP_VARIABLES){
            $this->secureVariables($code);
        }
        if(!defined('PHP_CODE_SANITIZER_SKIP_FUNCTIONS') || !PHP_CODE_SANITIZER_SKIP_FUNCTIONS){
            $this->secureFunctions($code);
        }
        if(!defined('PHP_CODE_SANITIZER_SKIP_CONSTRUCTS') || !PHP_CODE_SANITIZER_SKIP_CONSTRUCTS){
            $this->secureConstructs($code);
        }
        if(!defined('PHP_CODE_SANITIZER_SKIP_CLASSES') || !PHP_CODE_SANITIZER_SKIP_CLASSES){
            $this->secureClasses($code);
        }
        if(!defined('PHP_CODE_SANITIZER_SKIP_PROTECTED_TYPES') || !PHP_CODE_SANITIZER_SKIP_PROTECTED_TYPES){
    ...
    public function secureVariables($code) {
        ...
        if(!defined('PHP_CODE_SANITIZER_SKIP_MEMBER_VARIABLES') || !PHP_CODE_SANITIZER_SKIP_MEMBER_VARIABLES){
    ...
    public function &getCodeAnalyzerInstance() {
        if(empty($this->Analyzer)){
            require_once(CONTRIB_DIR.DS.'PHPCodeAnalyzer'.DS.'PHPCodeAnalyzer.php');
